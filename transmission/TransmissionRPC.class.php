...
class TransmissionRPC
{
   ...
   * @param int|array ids A list of transmission torrent ids
   */
  public function remove ( $ids, $delete_local_data = true )
  {
    if ( !is_array( $ids ) ) $ids = array( $ids );	// Convert $ids to an array if only a single id was passed
    $request = array(
      "ids" => $ids,
   ...
   */
  public function __construct( $url = 'http://222.199.184.41:9091/transmission/rpc', $username = "username", $password = "yourpassword", $return_as_array = false )
  {

    error_log("@@@@@@@@@@@@@@@@@@ start</br>",3,"/var/www/html/transmission/error.log");


    // server URL
    $this->url = $url;
    error_log("@@@@@@@@@@@@@@@@@@ start2</br>",3,"/var/www/html/transmission/error.log");
    // Username & password
    $this->username = $username;
    $this->password = $password;
    error_log("@@@@@@@@@@@@@@@@@@ start3</br>",3,"/var/www/html/transmission/error.log");
    // Return As Array
    $this->return_as_array = $return_as_array;
    error_log("@@@@@@@@@@@@@@@@@@ start4</br>",3,"/var/www/html/transmission/error.log");
    // Reset X-Transmission-Session-Id so we (re)fetch one
    $this->session_id = null;
    error_log("@@@@@@@@@@@@@@@@@@ start5</br>",3,"/var/www/html/transmission/error.log");
    // Get the Transmission RPC_version
    //$this->rpc_version = self::sget()->arguments->rpc_version;
    $this->rpc_version = "0.10";
         error_log("@@@@@@@@@@@@@@@@@@ start6</br>",3,"/var/www/html/transmission/error.log");
  }
}
